...
def normal():
    ...
    windows_normal = tk.Toplevel(bg="#B1A8A8")
    windows_normal.attributes('-fullscreen', True)

    def restart():
        windows_normal.destroy()
        normal()
    ...
    frame = tk.Frame(windows_normal)
    frame.pack()

    frame1 = tk.Frame(windows_normal, borderwidth=10, background="#B1A8A8", relief="sunken")
    frame1.pack(anchor="n", ipadx=275, pady=10)

    frame2 = tk.Frame(windows_normal, borderwidth=10, background="#B1A8A8", relief="sunken")
    ...
    def onclik(i,j,plateau_de_bombe, plateau_win):
        ...
        if plateau_de_bombe[i][j]==9:
            ...
            final_no = tk.Label(windows_normal, text="GAME OVER",bg="#B1A8A8", fg="blue", font=("Retro Gaming", 55))
            final_no.place(x=525, y=550)
            display_score = tk.Label(windows_normal, text="Ton Score :", bg="#B1A8A8", fg="blue", font=("Retro Gaming", 30))
            display_score.place(x=570, y=650)
            display_score_number = tk.Label(windows_normal, textvariable=appel_score, bg="#B1A8A8", fg="blue", font=("Retro Gaming", 30))
            display_score_number.place(x=860, y=650)
            exit_game = tk.Button(windows_normal, text="QUITTER", pady=11, relief="raised", bg="#B1A8A8", fg="blue", borderwidth=5, font=("Retro Gaming", 18), command=exit_windows)
        ...
        if verif(plateau_win) == True:
            ...
            appel_score = tk.IntVar()
            appel_score.set(nb_score)
            final_yes = tk.Label(windows_normal, text="WINNER", bg="#B1A8A8", fg="blue", font=("Retro Gaming", 55))
    ...
    btn_exit_game = tk.Button(windows_normal, text="QUITTER", pady=11, relief="raised", bg="#B1A8A8", fg="blue", borderwidth=5, font=("Retro Gaming", 18), command=exit_in_game)
    btn_exit_game.place(x=700, y=650)


    nb_bombe = 30
    nb_bombe_actuel = 0
    plateau_de_bombe = [[0 for j in range(12)] for i in range(8)]
    ...
    while nb_bombe_actuel < nb_bombe:
        ...
        position_bombe_column = random.randint(0, 11)
    ...
    plateau_win = [[0 for j in range(12)] for i in range(8)]
